            ...
            <div class="frameOuter container">
                <div class="row align-items-center">
                    ...
                    <div class="frame col-md-auto">
                        ...
                        <div id="app" class="{{  $data['sale_advisor'][0]->landingpage_version == 1 ? "lite" : "full" }} page-showme-list"  v-cloak>
                            <sales-header
                                ...
                                :breadcrumb="{
                                    title: '{{$data['breadcrumb_name']}}',
                                    link: '{{ url($data['sa_base_url'].$data['breadcrumb_url'] ) }}'
                                }"
                                :sadata="{{ $data['sale_advisor'][0] }}"
                                orglogo="{{$data['organisation']->logo}}"
                            ></sales-header>
                            <sales-show-item-list
                                :template="{{ $data['template'] }}"
                                :viewall="true"
                                :baselink="'{{ url($data['sa_base_url'].'/show') }}'"
                                :itemlist="{{ json_encode($data['item']) }}"
                            ></sales-show-item-list>
                            <sales-customfooter
                                :template="{{ $data['template'] }}"
                                :sadata="{{ $data['sale_advisor'][0] }}"
