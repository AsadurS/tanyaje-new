...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Tanyaje - @foreach($data['sale_advisor'] as $sa){{$sa->merchant_name}}@endforeach - Verify Me</title>
            ...
            <div class="frameOuter container">
                <div class="row align-items-center">
                    ...
                    <div class="frame col-md-auto">
                        ...
                        <div id="app" class="{{  $data['sale_advisor'][0]->landingpage_version == 1 ? "lite" : "full" }} page-verifyme"  v-cloak>
                            <sales-header
                                ...
                                :breadcrumb="{title:'Verification', link: '{{url($data['sa_base_url'])}}'}"
                                :verify="{{ $data['sale_advisor'][0]->verified }}"
                            ...
                            <sales-verify
                                ...
                                :template="{{ $data['template'] }}"
                                :sadata="{{ $data['sale_advisor'][0] }}"
                                orgdata_company_name="{{ $data['organisation']->company_name }}"
                                orgdata_brn_no="{{ $data['organisation']->brn_no }}"
                                orgdata_phone="{{ $data['organisation']->phone }}"
                                orgdata_corpphone="{{ $data['organisation']->corporate_phone }}"
                                orgdata_email="{{ $data['organisation']->email }}"
                                :verified="{{ $data['sale_advisor'][0]->verified }}"
                                :orgdata_bank1="{
                                    image: '{{ $data['organisation']->bank_1image }}',
                                    acc_name: '{{ $data['organisation']->bank_acc_name1 }}',
                                    acc_no: '{{ $data['organisation']->bank_acc_no1 }}',
                                }"
                                :orgdata_bank2="{
                                    image: '{{ $data['organisation']->bank_2image }}',
                                    acc_name: '{{ $data['organisation']->bank_acc_name2 }}',
                                    acc_no: '{{ $data['organisation']->bank_acc_no2 }}',
                                }"
                            ></sales-verify>
                            <sales-customfooter
                                :template="{{ $data['template'] }}"
