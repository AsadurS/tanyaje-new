...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Tanyaje - @foreach($data['sale_advisor'] as $sa){{$sa->merchant_name}}@endforeach - Campaign</title>
        @include('sales.common.opengraph_campaign')
        <!-- Fonts -->
    ...
    <body class="webapp" style="background-image:url('{{ asset('new/images/sales/bg.jpg') }}')">
            ...
            <div class="frameOuter container">
                <div class="row align-items-center">
                    ...
                    <div class="frame col-md-auto">
                        ...
                        <div id="app" class="{{  $data['sale_advisor'][0]->landingpage_version == 1 ? "lite" : "full" }} page-campaign"  v-cloak>
                            <sales-header
                                ...
                                :breadcrumb="{title:'Campaign', link: '{{url($data['sa_base_url'])}}/campaign'}"
                                :sadata="{{ $data['sale_advisor'][0] }}"
                                orglogo="{{$data['organisation']->logo}}"
                            ></sales-header>
                            <sales-campaign-detail
                                baseurl="{{ $data['sa_base_url'] }}"
                                :campaign="{{ json_encode($data['campaign']) }}"
                                :template="{{ $data['template'] }}"
                                :sadata="{{ $data['sale_advisor'][0] }}"
                                loadinganim="{{ asset('new/images/sales/loadingSpin.svg?v=1')}}"
                            ></sales-campaign-detail>
                            <sales-customfooter
                                :template="{{ $data['template'] }}"
                                :sadata="{{ $data['sale_advisor'][0] }}"
        ...
        @include('sales.common.scripts', [ 'openSticky' => true, 'shareTrack' => 'campaign_share'  ])
    </body>
</html>
