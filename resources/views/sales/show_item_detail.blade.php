...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Tanyaje - @foreach($data['sale_advisor'] as $sa){{$sa->merchant_name}}@endforeach - Show Me</title>
    ...
    <body class="webapp" style="background-image:url('{{ asset('new/images/sales/bg.jpg') }}')">
            ...
            <div class="frameOuter container">
                <div class="row align-items-center">
                    ...
                    <div class="frame col-md-auto">
                        ...
                        <div id="app" class="{{  $data['sale_advisor'][0]->landingpage_version == 1 ? "lite" : "full" }} page-showme-item-detail"  v-cloak>
                            <sales-header
                                ...
                                :breadcrumb="{
                                    title: '{{$data['item_detail'][0]['model_name']}}',
                                    link: '{{ url($data['sa_base_url'].'/show/model/'.$data['item_detail'][0]['model_id'] .'/items' ) }}'
                                }"
                                :sadata="{{ $data['sale_advisor'][0] }}"
                                orglogo="{{$data['organisation']->logo}}"
                            ></sales-header>
                            <sales-show-item-detail
                                :orgbrochure="'{{ $data['organisation']->brochure }}'"
                                :orgpricelist="'{{ $data['organisation']->pricelist }}'"
                                :baselink="'{{ url($data['sa_base_url'].'/show') }}'"
                                :item="{{ json_encode($data['item_detail'][0]) }}"
                                :template="{{ $data['template'] }}"
                                :askme="{{ $data['askme'] }}"
                            ></sales-show-item-detail>
                            <sales-customfooter
        ...
        <script>
            setTimeout(()=>{
                let extra_link = "<?= $data['item_detail'][0]['extra_link']?>";
                let extra_link_label = "<?= $data['item_detail'][0]['extra_link_label']?>";

                const targetDiv = document.querySelector('.right-content');
               if(targetDiv && extra_link){
                   const link = document.createElement('a');

                   link.id = 'link-button';
                   link.href = extra_link;
                   link.textContent = extra_link_label ? extra_link_label:'Extra Info';
                   link.classList.add('btn')
                   link.classList.add('btn-sm')

                   targetDiv.after(link);
               }
