...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Tanyaje - @foreach($data['sale_advisor'] as $sa){{$sa->merchant_name}}@endforeach - Ask Me</title>
            ...
            <div class="frameOuter container">
                <div class="row align-items-center">
                    ...
                    <div class="frame col-md-auto">
                        ...
                        <div id="app" class="{{  $data['sale_advisor'][0]->landingpage_version == 1 ? "lite" : "full" }} page-askme"  v-cloak>
                            <sales-header
                                ...
                                :breadcrumb="{title: '{{ $data['template'][0]->askme_title ?? "Ask Me" }}', link: '{{url($data['sa_base_url'])}}'}"
                                :sadata="{{ $data['sale_advisor'][0] }}"
                                orglogo="{{$data['organisation']->logo}}"
                            ></sales-header>
                        <sales-ask
                            :askme="{{ $data['askme'] }}"
                            :baselink="'{{ url($data['sa_base_url'].'/ask') }}'"
                            :orgbrochurelogo="'{{ $data['organisation']->brochure_logo }}'"
                            :orgpricelistlogo="'{{ $data['organisation']->pricelist_logo }}'"
                            :orgextrabrochurelogo="'{{ $data['organisation']->extra_brochure_logo }}'"
                            :orgextrapricelistlogo="'{{ $data['organisation']->extra_pricelist_logo }}'"
                            :orgextrabrochurelabel="'{{ $data['organisation']->extra_brochure_label }}'"
                            :orgextrapricelistlabel="'{{ $data['organisation']->extra_pricelist_label }}'"
                            :orgpromotionlogo="'{{ $data['organisation']->promotion_logo }}'"
                            :orgbrochure="'{{ $data['organisation']->brochure }}'"
                            :orgpricelist="'{{ $data['organisation']->pricelist }}'"
                            :orgpromotion="'{{ $data['organisation']->promotion }}'"
                            :template="{{ $data['template'] }}"
                        ></sales-ask>
                            <sales-customfooter
                                :template="{{ $data['template'] }}"
