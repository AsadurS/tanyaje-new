      ...
      <a href="{{ URL::to('admin/sale_advisors/dashboard')}}" class="logo">
        ...
        <span class="logo-lg">
            @if( Auth()->user()->role_id == \App\Models\Core\User::ROLE_SUPER_ADMIN || Auth()->user()->role_id == \App\Models\Core\User::ROLE_NORMAL_ADMIN )
              <b>{{ trans('labels.admin') }}</b>
            @elseif( Auth()->user()->role_id == \App\Models\Core\User::ROLE_MERCHANT  )
                <b>Merchant</b>
            @elseif( Auth()->user()->role_id == \App\Models\Core\User::ROLE_MANAGER )
                <b>Manager</b>
            @endif
        </span>
      </a>
          ...
          <li class="dropdown user user-menu">
            <a href="#" class="dropdown-toggle" data-toggle="dropdown">
              ...
              <span class="hidden-xs">
                @if(Auth::guard('saleadvisor')->check())
                  {{ Auth::guard('saleadvisor')->user()->merchant_name }} 
                @else
            ...
            <ul class="dropdown-menu">
              ...
              <li class="user-header">
                ...
                <p>
                  @if(Auth::guard('saleadvisor')->check())
                    {{ Auth::guard('saleadvisor')->user()->merchant_name }} 
                    <small>{{ trans('labels.SaleAdvisor')}}</small>
                  @else
                    {{ auth()->user()->first_name }} {{ auth()->user()->last_name }}
              ...
              <li class="user-footer">
                ...
                <div class="pull-right">
                  @if(Auth::guard('saleadvisor')->check())
                    <a href="{{ URL::to('sale_advisor/logout')}}" class="btn btn-default btn-flat">{{ trans('labels.sign_out') }}</a>
                  @else
