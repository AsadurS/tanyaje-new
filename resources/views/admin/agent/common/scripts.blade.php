...
$(document).on('click', '#addBranch', function(e){
	$("#loader").show();
	var formData = $('#addBranchFrom').serialize();
    var error = "";
...
if(error==""){
	$.ajax({
		url: "{{url('admin/merchants/addmerchantbranch')}}",
		type: "POST",
		data: formData,
		async: false,
		success: function (res) {
			...
			if(res.length != ''){
				...
				for (i = 0; i < res.length; ++i) {
					...
					showData[i] = "<tr><td>"+j+"</td><td><strong>Merchant Name:</strong> "+res[i].entry_merchant_name+"<br><strong>Phone:</strong> "+res[i].entry_phone+"</td><td><strong>State:</strong> "+res[i].state_name+"<br><strong>City:</strong> "+res[i].city_name+"</td><td><a class='badge bg-light-blue editBranchModal' customers_id = '"+res[i].customers_id+"' branch_id = '"+res[i].branch_id+"' ><i class='fa fa-pencil-square-o' aria-hidden='true'></i></a><a customers_id = '"+res[i].customers_id+"' branch_id = '"+res[i].branch_id+"' class='badge bg-red deleteBranchModal'><i class='fa fa-trash' aria-hidden='true'></i></a></td></tr>";
...
//editBranchModal
$(document).on('click', '.editBranchModal', function(){
	var user_id = $(this).attr('user_id');
	var branch_id = $(this).attr('branch_id');
	$.ajax({
		url: "{{url('admin/merchants/editbranch')}}",
		type: "POST",
		data: '&user_id='+user_id+'&branch_id='+branch_id,
		success: function (data) {
			$('.editContent').html(data);
...
//CopyBranchModal
$(document).on('click', '.CopyBranchModal', function(){
	var branch_id = $(this).attr('branch_id');
    $(".branch_input").val(branch_id);
    $(".BranchContent option").removeAttr('disabled');
    $(".BranchContent option[value='"+branch_id+"']").attr('disabled',true);
	$('#CopyBranchModal').modal('show');
});

//ajax call for submit value
$(document).on('click', '#CopyBranch', function(e){
	$("#loader").show();
	var formData = $('#CopyBranchFrom').serialize();
    var error = "";

    //to validate text field
    $(".c-field-validate").each(function() {
...
$(document).on('click', '#updateAddress', function(e){
	...
	$(document).on('click', '#updateBranch', function(e){
		...
		$("#loader").show();
		var formData = $('#editBranchFrom').serialize();
		...
		if(error=="") {
			...
			$.ajax({
				url: "{{url('admin/merchants/updatebranch')}}",
				type: "POST",
				data: formData,
	...
	//deleteBranchModal
	$(document).on('click', '.deleteBranchModal', function(){
		var customers_id = $(this).attr('customers_id');
		var branch_id = $(this).attr('branch_id');
		$('#customers_id').val(customers_id);
...
{{--{{ URL::to('admin/deleteproductattributemodal')}}--}}
	...
	//deletemakeModal
	$(document).on('click', '#makeFrom', function(){
	...
	//deletemodelModal
	$(document).on('click', '#modelFrom', function(){
	...
	//deletestateModal
	$(document).on('click', '#stateFrom', function(){
	...
	//deletetypeModal
	$(document).on('click', '#typeFrom', function(){
	...
	//deleteCarModal
	$(document).on('click', '#deleteCarId', function(){
	...
	//deleteMemberModal
	$(document).on('click', '#deleteMemberFrom', function(){
		var users_id = $(this).attr('users_id');
		$('#users_id').val(users_id);
	...
	//deleteManagerModal
	$(document).on('click', '#deleteManagerFrom', function(){
		var users_id = $(this).attr('users_id');
		$('#users_id').val(users_id);
	...
	$(document).on('change', '#choose_make_id', function(e){

		var make_id = $(this).val();
		$.ajax({
			url: "{{ URL::to('admin/getModel')}}",
			dataType: 'json',
			type: "post",
			data: {
				"_token": "{{ csrf_token() }}",
			...
			success: function(data){
				if(data.data.length>0){
					var i;
					var custom = "<option value=''> Select Model </option>";
					var showData = [];
					for (i = 0; i < data.data.length; ++i) {
						showData[i] = "<option value='"+data.data[i].model_id+"'>"+data.data[i].model_name+"</option>";
					}
					showData.unshift(custom)
					$('.selectmodel').show();
				}else{
					showData = "<option value=''></option>"
					$('.selectmodel').hide();
	...
	$(document).on('change', '#choose_state_id', function(e){
		...
		$.ajax({
			...
			success: function(data){
				if(data.data.length>0){
					...
					var showData = [];
					for (i = 0; i < data.data.length; ++i) {
						showData[i] = "<option value='"+data.data[i].city_id+"'>"+data.data[i].city_name+"</option>";
					}
					$('.selectcity').show();
				}else{
					showData = "<option value=''></option>"
...
//Bulk Update Airtime
$(document).on('change', '#merchant-airtime', function(){
	const merchantId = $(this).val();

	$('#airtime-cars-options')
		.find('option')
		.remove();

	$.get('{{ URL::to("admin/car/bulk-airtime/get-car")}}' + `?merchant_id=${merchantId}`, function(res) {
		res.data.forEach(car => {
			$('#airtime-cars-options')
				.find('option')
				.end()
				.append(`<option value="${car.car_id}"  ${car.is_airtime_hide ? 'selected' : ''}>${car.title}</option>`)
		});

		if (res.data.length > 0) {
			$('#airtime-cars-options').show();
		}

		setMultiselectCarAirtime();
	});

});

//Function generate multiple select for bulk edit airtime
function setMultiselectCarAirtime()
{
	//Handle Page Bulk Edit Airtime
	$('#airtime-cars-options').multiSelect({
		selectableHeader: '<div style="text-align:center;font-weight:bold">Airtime Show</div><input type="text" class="search-input" autocomplete="off" style="width: 100%;margin-bottom:5px">',
		selectionHeader: '<div style="text-align:center;font-weight:bold">Airtime Hide</div><input type="text" class="search-input" autocomplete="off" style="width: 100%;margin-bottom:5px">',
		...
		afterSelect: function(id){
			this.qs1.cache();
			this.qs2.cache();
			$.get('{{ URL::to("admin/car/bulk-airtime/toggle-airtime/")}}' + `/${id}`);
		},
	...
	$('#airtime-cars-options').multiSelect('refresh');
}
</script>
...
<script>
jQuery(document).ready(function(){
    jQuery(".remove_banner").on("click",function(){
        jQuery("#oldBanner").val("");
